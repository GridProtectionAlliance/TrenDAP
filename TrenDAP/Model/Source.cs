...
namespace TrenDAP.Model
{
    ...
    public class SourceHelper<T>: XDAAPIHelper where T: Source, new()
    {
        private T m_source;

        public SourceHelper(IConfiguration config, int dataSourceId)
        {
            ...
            {
                m_source = new Gemstone.Data.Model.TableOperations<T>(connection).QueryRecordWhere("ID = {0}", dataSourceId);
            }
        }

        public SourceHelper(T source)
        {
            m_source = source;
        }
        ...
        protected override string Key
        {
            get
            {
                return m_source.RegistrationKey;
            }
