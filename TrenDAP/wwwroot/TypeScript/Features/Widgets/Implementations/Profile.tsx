...
import { CheckBox, ColorPicker, Input, Select } from '@gpa-gemstone/react-forms';
...
export const ProfileWidget: WidgetTypes.IWidget<ISettings, IChannelSettings, null> = {
    Name: "Profile",
    DefaultSettings: { XAxisLabel: 'Percent', YAxisLabel: 'Value', BinCount: 15 },
    ...
    WidgetUI: (props) => {
        ...
        function Initialize() {
            ...
            const total = allBins.reduce((acc, cur) => acc + cur.bin.length, 0);
            const yMax = d3.max(allBins, d => d.bin.x1);
            const yMin = d3.min(allBins, d => d.bin.x0);
            let xMax = d3.max(allBins, d => d.bin.length) / total;
            ...
            svg.append("g")
                ...
                .attr("opacity", 0.5)
                .attr("x", d => xScale(0))
                .attr("y", d => yScale(d.bin.x1))
                .attr("width", d => xScale(d.bin.length / total) - xScale(0))
                .attr("height", d => yScale(d.bin.x0 as number) - yScale(d.bin.x1 as number));

            // Plot profile lines
            if (profiles.length > 0) {

                const lineFunc = d3.line<{ Bin: number, Value: number }>().x(dd => xScale(dd.Value)).y(dd => yScale(dd.Bin));
    ...
    SettingsUI: (props) => {
        return <>
            <Input<ISettings> Field='XAxisLabel' Label="X Axis Label" Record={props.Settings} Type='text' Setter={(r) => props.SetSettings(r)} Valid={() => true} />
            <Input<ISettings> Field='BinCount' Label='Bins' Record={props.Settings} Type='number' Setter={(r) => props.SetSettings(r)} Valid={() => true} />
        </>
    },
                ...
                <ReactTable.Table<DataSetTypes.IDataSetMetaData>
                    ...
                    OnClick={(item) => {
                        ...
                        if (isSelected)
                            ...
                            props.AddChannel(item.row.ID, ProfileWidget.DefaultChannelSettings);
                    }}
