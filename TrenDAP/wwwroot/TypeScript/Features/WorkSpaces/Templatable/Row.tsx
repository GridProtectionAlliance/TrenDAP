...
import Histogram from '../../Widgets/Templatable/Histogram';
import Profile from '../../Widgets/Templatable/Profile';
import Stats from '../../Widgets/Templatable/Stats';
import Table from '../../Widgets/Templatable/Table';
import Text from '../../Widgets/Templatable/Text';
import Trend from '../../Widgets/Templatable/Trend/Trend';
import XvsY from '../../Widgets/Templatable/XvsY';
import { CreateWidget } from '../../Widgets/Templatable/Implementations';
const NavMargin = 85;
const NavWidth = 200;

export default function Row(props: TrenDAP.iTemplatableRow) {
    const [toggle, setToggle] = React.useState<boolean>(false);
    const [height, setHeight] = React.useState<number>(props.Height);

    function HandleAddObject(type: TrenDAP.WidgetType) {
        let row = { ...props, Update: undefined, WorkSpace: undefined, RemoveRow: undefined } as TrenDAP.iTemplatableRow;
        row.Widgets.push(CreateWidget(type, props.By, props.Device, 100,(window.innerWidth - NavWidth) / 2))
        props.Update(row);
    }
                ...
                <div className='grid-container' style={{ display: 'inline-flex' }}>
                    {(props.Widgets as any[]).map((widget, index) => <Widget {...{
                        ...
                        Update: (newRecord) => {
                            ...
                            row.Widgets[index] = widget as any;
                            props.Update(row);
                        },
                        Remove: () => {
                            let row = { ...props };
...
const Widget: React.FunctionComponent<TrenDAP.iTemplatableWidget> = (props) => {
